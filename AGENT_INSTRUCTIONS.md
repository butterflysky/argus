...
- Permission gate: OPs bypass; linked users need `hasAccess`; legacy vanilla-whitelisted users are kicked immediately with a link token (no full login); strangers are denied with the application message (plus optional invite link).
